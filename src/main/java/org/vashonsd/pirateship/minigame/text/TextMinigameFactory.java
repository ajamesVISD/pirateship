 ...
 * we stuff into each Location is not an instance a TextMinigame; it is an offer to produce one. We do this for
 * two reasons:
 * 
 * 1) The TextMinigameFactory can describe the game, including its name and description and so forth, without
 ...
 * an object that represents limited access to Player state, so that the TextMinigame can find out the Player's name,
 * or do money dealings with the Player.
 */
public interface TextMinigameFactory {
	/*
	...
	/*
	 * Announce() provides the String by which the TextMinigame announces its presence in a Location.
	 * 
